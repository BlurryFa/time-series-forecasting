...
class CnnTS():
    ...
    def initialize(self, n_features=1):
        if self.regressor is None:
            self.regressor = Sequential()
            self.regressor.add(Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(self.step, n_features)))
            self.regressor.add(MaxPooling1D(pool_size=2))
            self.regressor.add(Flatten())
            self.regressor.add(Dense(50, activation='relu'))
    ...
    def train(self, X, y):
        ...
        X = X.reshape((X.shape[0], self.step, 1))
    ...
    def plot_model(self):
        plot_model(self.regressor, to_file='neural_network_model/cnn.pdf', show_shapes=True)
    ...
    def predict(self, X):
        X = X.reshape((X.shape[0], self.step, 1))
        y_pre = self.regressor.predict(X, verbose=0)
